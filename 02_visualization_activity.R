...
# Filter activity type with non-NA Action
activity_type <- 
  data |>
  filter(!is.na(Action)) |> 
  mutate(TaskLabel = paste("CFR 12 ", sub(" .*", "", label),
...
# Function to generate plots by category
generate_plot <- function(task_info, reassigned_tasks, category, title) {
  ...
  p <- ggplot() +
    ...
    geom_point(data = filtered_reassigned_tasks, 
               aes(x = Year, y = label), 
               shape = 4, size = 3) +
    geom_point(data = activity_type, aes(x = Year, y = TaskLabel, shape = Action), size = 1.5) +
    scale_shape_manual(values = c("Addition" = 3, "Deletion" = 4, 
                                  "Modification" = 2, "Unknown" = 0, "Other" = 5)) +
...
# Creating plots by category
consumer_reg_plot <- generate_plot(task_info, reassigned_tasks, "consumer regulation", "Consumer Regulation") +
  theme(axis.text.x = element_blank(),
        legend.position = "none")

monetary_pol_plot <- generate_plot(task_info, reassigned_tasks, "monetary policy", "Monetary Policy") +
  theme(axis.text.x = element_blank(),
        legend.position = "none")

prudential_reg_plot <- generate_plot(task_info, reassigned_tasks, "prudential regulation", "Prudential Regulation") +
  labs(y = "Task") +
  theme(axis.text.x = element_blank(),
...
other_plot <- generate_plot(task_info, reassigned_tasks, "other", "Other") +
  labs(x = "Year") +
  theme(legend.position = "none")

# Adding title to the combined plot
title <- ggplot() +
...
ggsave("combined_plot_activity.jpg", plot = combined_plot, width = 6.5, height = 9)
