...
# Load the data
data <- read_csv("stacked_data_cleaned_activity.csv")
...
colnames(data) <- c("CFR.Part", "Name.of.the.Part", "Year", 
                    ...
                    "lv_distances", "presumed_same", "same_next_edition", "category", "Action")

# Setup the data frame similar to the original example
map <- data |>
  select(Year, CFR.Part, Next.Edition, same_next_edition, category, Action) |>
  rename(Task = CFR.Part, Next.Year = Next.Edition, Next.Task = same_next_edition, Category = category, Action = Action)
...
final_labels <- labels |> select(Year, Task, label, Category, Action)
final_labels

write_csv(final_labels, file = "labels_test.csv")
